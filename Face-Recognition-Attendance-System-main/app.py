...
def add():
    ...
    for user in existing_users:
        user_roll = user.split('_')[-1]
        if newuserid == user_roll:
            # If roll number already exists, return with an error message
            error_message = f"Roll number {newuserid} already exists. Please use a different roll number."
            return render_template('home.html', error=error_message, totalreg=totalreg(), datetoday2=datetoday2)

    # Continue with adding the new user if roll number is unique
    ...
    while i < 10:
        ...
        for (x, y, w, h) in faces:
            ...
            if j % 10 == 0:
                ...
                i += 1
                time.sleep(0.5)  # Add a delay of __ seconds between capturing each photo
            j += 1
        if j == 500:
            break
